...
type Screen = 'dev'|'splash'|'choix'|'signup'|'photo'|'appart'|'questionnaire'|'interests'|'ia_analyse'|'groupe_propose'|'groupe_accepte'|'groupe_actif'|'activites'|'notifs'|'profil'|'voisinage'|'depot_annonce'|'groupes_bailleur'|'dashboard_bailleur';
...
export default function App() {
  ...
  const renderScreen = () => {
    switch (screen) {
      ...
      case 'signup': return <SignUpScreen onNext={() => go('photo')} onBack={back} />;
      ...
      case 'groupe_actif': return <GroupeActifScreen onActivites={() => go('activites')} onProfil={() => go('profil')} onNotifs={() => go('notifs')} onVoisinage={() => go('voisinage')} onTournante={() => {}} />;
      case 'activites': return <ActivitesScreen onBack={back} onGroupe={() => go('groupe_actif')} onProfil={() => go('profil')} onNotifs={() => go('notifs')} onVoisinage={() => go('voisinage')} />;
      case 'notifs': return <NotifsScreen onGroupe={() => go('groupe_actif')} onActivites={() => go('activites')} onProfil={() => go('profil')} onVoisinage={() => go('voisinage')} />;
      case 'profil': return <ProfilReputationScreen onGroupe={() => go('groupe_actif')} onActivites={() => go('activites')} onNotifs={() => go('notifs')} onVoisinage={() => go('voisinage')} />;
